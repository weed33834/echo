...
Echo 不是算命软件，而是一套**假设验证工具**。

用八字、紫微、六爻等体系排出的结果，本质上是一个**预测**。Echo 做的事情是：记录这些预测，设定印证时间节点，到期后回来复盘——应验了还是落空了。随着复盘次数累积，你的「命格可信度」逐步建立，也能看清哪套体系对自己更准。
...
- 多模型支持：DeepSeek / OpenAI / Claude / 通义千问 / Ollama 本地模型，统一走 OpenAI 兼容协议，SSE 流式输出
- 多轮推理：AI 自主调用 18 种命理工具，拿到结构化结果后继续推理，循环守卫防止无限调用
- 上下文记忆：多会话持久化于 localStorage，自动注入用户档案、八字数据、最近推演历史、命理知识库与当前节气
- 联网搜索：集成 Tavily API，AI 可获取实时信息
- 安全沙箱：输入净化、注入检测、参数校验、超时控制、输出验证；五类动态安全护栏（医疗 / 法律 / 财务 / 情感 / 危机）
- 证据导向：要求 AI 引用工具数据作答，跨工具印证不同体系的结果，不做绝对断言
...
**零运行时依赖**——除 Vue / Pinia / Router 三个核心库外，其余功能均自实现：
